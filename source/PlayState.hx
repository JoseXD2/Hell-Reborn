...
import FunkinLua;
// import FunkinLua;
import PauseSubState2.ShakyText as ShakyText;
...
class PlayState extends MusicBeatState
{
	...
	public var items:Map<String, Map<String, Dynamic>> = [
		"Sprites" => [],
		"Texts" => [],
		"Sounds" => [],
		"Tweens" => [],
	...
	public var notesMS:Array<Note.NoteUtil>;
	...
	public var redGradThing:Bool = false;
	public var redGradSpeed:Float = 1;
	private var redGradSPR:FlxSprite;
	private var redGradTwn:FlxTween;
	...
	override public function create()
	{
		...
		PauseSubState2.songName = null; //Reset to default
		...
		#if desktop
		storyDifficultyText = (storyDifficulty == 3 ? 'Encore' : CoolUtil.difficulties[storyDifficulty]);
		...
		healthBar1.visible = !ClientPrefs.hideHud;
		...
		iconP1.y = (FlxG.height/2)-50;
		iconP1.visible = !ClientPrefs.hideHud;
		iconP1.alpha = ClientPrefs.healthBarAlpha;
		add(iconP1);

		iconP2 = new HealthIcon(dad.healthIcon, false);
		iconP2.y = (FlxG.height/2)-50;
		iconP2.visible = !ClientPrefs.hideHud;
		iconP2.alpha = ClientPrefs.healthBarAlpha;
		add(iconP2);
		reloadHealthBarColors();

		scoreTxt = new FlxText(50, FlxG.height-160, 0, "Score: N/A", 40);
		...
		{
			timeTxt.text = 'Song:  '+SONG.song+' - '+storyDifficultyText.toUpperCase();
		}
		updateTime = true;

		missesTxt = new FlxText(40, FlxG.height-260, 0, "Misses: N/A", 40);
		...
		botplayTxt = new FlxText(55, FlxG.height-110, 0, "BOTPLAY", 40);
		botplayTxt.setFormat(Paths.font("Futura.otf"), 32, 0xFF19181a, LEFT, FlxTextBorderStyle.OUTLINE, 0xFFe8d8c0);
		...
		if (PauseSubState2.songName != null) {
			precacheList.set(PauseSubState2.songName, 'music');
		...
		redGradSPR = flixel.util.FlxGradient.createGradientFlxSprite(FlxG.width, FlxG.height, [0xFFff0000, 0x00000000, 0xFFff0000]);
		redGradSPR.cameras = [camHUD];
		redGradSPR.alpha = 0;
		add(redGradSPR);
	...
	public function addTextToDebug(text:Dynamic, color:FlxColor) {
		#if LUA_ALLOWED
	...
	public function reloadHealthBarColors() {
		healthBar1.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
		healthBar2.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
	}
	...
	{
		scoreTxt.text = 'Score:     $songScore'+(ratingName != '?' ? ' - '+ratingFC : '');
		missesTxt.text = 'Misses:   $songMisses';
		scoreTxt.x = (ClientPrefs.downScroll) ? 60 : 50;
		scoreTxt.y = (ClientPrefs.downScroll) ? 140 : FlxG.height-160;
		...
		if(ClientPrefs.scoreZoom && !miss && !cpuControlled)
		{
			...
			scoreTxt.alpha = 0;
			scoreTxtTween = FlxTween.tween(scoreTxt, {alpha: 1}, 0.2, {
				onComplete: function(twn:FlxTween) {
					scoreTxtTween = null;
				}
			});
	...
	function startSong():Void
	{
		...
		FlxTween.tween(timeTxt, {"scale.x": 0.5}, 0.2, {ease: FlxEase.elasticOut, type: BACKWARD});
		...
		if (sys.FileSystem.exists('mods/data/'+Paths.formatToSongPath(SONG.song).toLowerCase()+'/credits-'+storyDifficultyText.toLowerCase()+'.txt') || sys.FileSystem.exists('assets/data/'+Paths.formatToSongPath(SONG.song).toLowerCase()+'/credits-'+storyDifficultyText.toLowerCase()+'.txt')){
			openCredits(Paths.getTextFromFile('data/'+Paths.formatToSongPath(SONG.song).toLowerCase()+'/credits-'+storyDifficultyText.toLowerCase()+'.txt'));
		}else{
			if (sys.FileSystem.exists('mods/data/'+Paths.formatToSongPath(SONG.song).toLowerCase()+'/credits.txt') || sys.FileSystem.exists('assets/data/'+Paths.formatToSongPath(SONG.song).toLowerCase()+'/credits.txt'))
				openCredits(Paths.getTextFromFile('data/'+Paths.formatToSongPath(SONG.song).toLowerCase()+'/credits.txt'));
		}
	}
	...
	{		
		...
		var songNameThing:ShakyText = new ShakyText(-350, -200, 0, Paths.formatToSongPath(SONG.song).toUpperCase(), 100, Paths.font('Futura.otf'), 0xFFe9a642, 0xFFac0001, 5);
		...
		var creditsThing2:FlxText = new FlxText(-550, 200, 0, '$textBlah\n', 35);
		...
		FlxTween.tween(something, {x: -20}, 0.3, {ease: FlxEase.expoIn, onComplete: function(posi:FlxTween){
			FlxTween.tween(something2, {y: -70}, 0.2, {ease: FlxEase.expoIn, onComplete: function(paru:FlxTween){
				...
				FlxTween.tween(something2, {"scale.y": 1.5}, 0.2, {ease: FlxEase.elasticOut, type: BACKWARD});
				FlxTween.tween(something, {"scale.y": 0.6}, 0.2, {ease: FlxEase.elasticOut, type: BACKWARD});
	...
	private function generateSong(dataPath:String):Void
	{
		...
		notesMS = new Array<Note.NoteUtil>();
	...
	override public function update(elapsed:Float)
	{
		...
		if (!redGradThing){
			...
			if (redGradTwn == null)
				redGradTwn = FlxTween.tween(redGradSPR, {alpha: 1}, 1/redGradSpeed, {ease: FlxEase.smoothStepInOut, type: PINGPONG});
			else
		...
		bestCombo = Std.int(Math.max(bestCombo, combo));
	...
	function openPauseMenu()
	{
		...
		openSubState(new PauseSubState2(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
		PauseSubState2.songArray = [Paths.formatToSongPath(SONG.song).toUpperCase(), storyDifficultyText];
		//}
	...
	{
		if (!HitGraph.ResultScreen.force && !isStoryMode){
			persistentUpdate = false;
			persistentDraw = true;
			if(FlxG.sound.music != null) {
				FlxG.sound.music.pause();
			...
			openSubState(new HitGraph.ResultScreen());
			trace('im open!!');
		}else{
			trace('im closed!!');
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		notesMS.insert(notes.members.indexOf(daNote), new Note.NoteUtil(daNote.strumTime, Conductor.songPosition, daNote.noteData, daNote.isSustainNote, 'miss'));
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		// var penis:Array<Dynamic> = (SONG.notes[curSection].gfSection ? [gf, girlfriendCameraOffset[0], girlfriendCameraOffset[1]] : [dad, opponentCameraOffset[0] + 150, opponentCameraOffset[1] - 100]);
		// if (!SONG.notes[curSection].gfSection) camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
		// if (Math.abs(note.noteData) == 0 || Math.abs(note.noteData) == 3) camFollow.x = penis[0].getMidpoint().x + penis[0].cameraPosition[0] + (penis[1][0] + 150) + (Math.abs(note.noteData) == 0 ? -25 : 35);
		// if (Math.abs(note.noteData) == 1 || Math.abs(note.noteData) == 2) camFollow.y = penis[0].getMidpoint().y + penis[0].cameraPosition[1] + (penis[1][1] - 100) + (Math.abs(note.noteData) == 1 ? 35 : -35);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			notesMS.insert(notes.members.indexOf(note), new Note.NoteUtil(note.strumTime, Conductor.songPosition, leData, isSus, note.rating));
			callOnLuas('goodNoteHit', [notes.members.indexOf(note), leData, leType, isSus]);
			// var penis:Array<Dynamic> = (SONG.notes[curSection].gfSection ? [gf, girlfriendCameraOffset[0], girlfriendCameraOffset[1]] : [boyfriend, boyfriendCameraOffset[0] - 100, boyfriendCameraOffset[1] - 100]);
			// // if (!SONG.notes[curSection].gfSection) camFollow.set(boyfriend.getMidpoint().x - 100, boyfriend.getMidpoint().y - 100);
			// if (Math.abs(note.noteData) == 0 || Math.abs(note.noteData) == 3) camFollow.x = penis[0].getMidpoint().x - penis[0].cameraPosition[0] + (penis[1]) + (Math.abs(note.noteData) == 0 ? -45 : 45);
			// if (Math.abs(note.noteData) == 1 || Math.abs(note.noteData) == 2) camFollow.y = penis[0].getMidpoint().y + penis[0].cameraPosition[1] + (penis[2]) + (Math.abs(note.noteData) == 1 ? 45 : -45);
			{
				note.kill();
				notes.remove(note, true);
				note.destroy();
...
class CherryBar extends FlxSprite{
	...
	{
		lerpedAmount = FlxMath.lerp(lerpedAmount, currentAmount, CoolUtil.boundTo(elapsed * 30, 0, 1));
